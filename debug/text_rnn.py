...
class TextRNN(object):
    def __init__(
      ...
      embedding_size, l2_reg_lambda, num_layers= 2, hidden_dim = 64, rnn="lstm", trainable=True):
        ...
        self.input_x = tf.placeholder(tf.float32, [None, sequence_length, embedding_size], name="input_x")# 为输入的数据创建占位符，在任何阶段都可使用它向模型输入数据。第二个参数是输入张量的形状，none表示该维度可以为任意值。而在我们模型中该维度表示批处理大小默认为64。
